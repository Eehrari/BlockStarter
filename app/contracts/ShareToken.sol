...
contract ShareToken {
    ...
    // Set params
    function setParams(uint256 _initialSupply, string _tokenName, uint8 _decimalUnits, string _tokenSymbol)  {
        initialSupply = _initialSupply;
        name = _tokenName;
        decimals = _decimalUnits ;
        symbol=_tokenSymbol;
    }
    // Return all shareToken information
    function getInfo() constant returns(uint256, string, uint8, string) {
        return(initialSupply, name, decimals, symbol);
    }
    // Return the balance of creator
    function tokenLeft() constant returns(uint256) {
        return balanceOf[creator];
    }
    // Show the intial supply and creator balance share the same values
    ...
    //Reciver balance
    function reciverBalance(address _to) constant returns(uint256) {
